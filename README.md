...
| **Poet SRPO** | ✓ | Self-Refinement Policy Optimization: dual loss (40% generation + 60% revision). Trains poet to both generate and self-revise. L = α·L_gen + (1-α)·L_rev + β·KL. |
...
| 8 | Train final educator + poet (SFT; optional SRPO/REINFORCE Stage 2) | 0 | Modal GPU |
...
**SRPO (Stage 2):** If using SRPO instead of REINFORCE, generate trajectories with `generate_srpo_data.py` using **Llama 4 Maverick via Bedrock** for all stages: draft_0 (initial poem) → critique (Educator persona) → draft_1 (gold revision). Trajectories are filtered by improvement (`reward_1 - reward_0 >= 0.05`) and quality (`reward_1 >= 0.5`). The SRPO-trained poet learns both generation (40% loss weight) and self-revision (60% loss weight) in a single training run, enabling critique-responsive revision at inference without additional Educator calls.
...
| `generate_srpo_data` (Stage 2) | poet prompts | SRPO trajectories (Llama 4 Maverick) |
...
# Optional: SRPO trajectories (Stage 2 - requires AWS Bedrock + Llama 4 Maverick access)
...
# Poet (Stage 2: REINFORCE — optional, after SFT)
modal run scripts/modal/train_poet_reinforce.py [--num-epochs-override N]
modal run scripts/modal/export_gguf.py::export_poet_reinforce

# Orchestrate
modal run scripts/modal/modal_app.py [--educator-only] [--poet-only] [--reinforce] [--train-only] [--num-epochs N]
```
...
python scripts/sagemaker/train_sagemaker.py --task reinforce [--num-epochs-override N]  # Stage 2
...
# Or: modal run scripts/modal/modal_app.py --backend sagemaker [--educator-only] [--poet-only] [--reinforce] [--num-epochs N]
```
...
**Stage 2 (optional):** Choose between two approaches:

- **SRPO (recommended):** Self-Refinement Policy Optimization. Trains the poet to both generate AND self-revise based on critique using dual-objective loss: 40% generation (draft_0) + 60% revision (draft_1 | critique). Trajectories (prompt → draft_0 → critique → draft_1) generated via Llama 4 Maverick on Bedrock, filtered by improvement. At inference, poet self-revises without additional Educator calls. Config: `config/srpo_training.yaml`. See `scripts/data/generate_srpo_data.py`.

- **REINFORCE (legacy):** Reward-weighted regression on the SFT checkpoint. Generates N completions per prompt, scores each with the deterministic `rhyme_analyzer.py` (partial credit: perfect 1.0, slant 0.6, assonance 0.3, none 0.0), normalizes advantages, and trains with KL penalty against the frozen SFT ref. Config: `config/reinforce_training.yaml`.

**For detailed comparison (SRPO vs REINFORCE):** See [scripts/training/README.md](scripts/training/README.md#comparison-srpo-vs-reinforce)
...
| `config/srpo_data_generation.yaml` | SRPO trajectory generation (Llama 4 Maverick via Bedrock) |
| `config/reinforce_training.yaml` | Stage 2: REINFORCE reward-weighted regression (legacy) |
