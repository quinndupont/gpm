...
| **Poet REINFORCE** | ✓ | Reward-weighted regression: score poems with deterministic algo, Â = (score−μ)/σ, L = −Σ Â·log P. KL penalty to SFT baseline. |
...
**Stage 2 (REINFORCE, optional):** Reward-weighted regression on the SFT checkpoint. Generates N completions per prompt, scores each with the deterministic `rhyme_analyzer.py` (partial credit: perfect 1.0, slant 0.6, assonance 0.3, none 0.0), normalizes advantages, and trains with KL penalty against the frozen SFT ref. Config: `config/reinforce_training.yaml`. Eval: `rhyme_analyzer.py`, `meter_analyzer.py`, `form_registry.py`.
...
| `config/reinforce_training.yaml` | Stage 2: REINFORCE reward-weighted regression |
