...
**Optional:** REINFORCE stage on poet: `config/reinforce_training.yaml`, `train_poet_reinforce.py`, data in `data/rhyme_training/`. For 32B poet when both models don’t fit in memory, use `scripts/inference/swapping_pipeline.py` (load one model at a time).
