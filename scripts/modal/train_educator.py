...
CONFIG_PATH = ROOT / "config" / "educator_training.yaml"
...
image = (
    ...
    .add_local_file(str(ROOT / "config" / "educator_training.yaml"), "/config/educator_training.yaml")
)
...
def train_educator(num_epochs_override: int | None = None):
    import torch
    ...
    cfg = yaml.safe_load(Path("/config/educator_training.yaml").read_text())
    ...
    save_path = Path(ckpt_cfg.get("save_path", "/vol/checkpoints/educator/"))

    # Load data from volume
    train_file = Path("/vol/data/educator_train.jsonl")
    valid_file = Path("/vol/data/educator_valid.jsonl")
    data_vol.reload()
    ...
    if not train_data:
        raise ValueError("No training data in educator_train.jsonl")
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    with app.run():
        path = train_educator.remote(num_epochs_override=a.num_epochs_override)
        print(f"Done: {path}")
