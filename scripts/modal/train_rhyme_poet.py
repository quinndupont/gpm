...
def train_rhyme_poet(num_epochs_override: int | None = None):
    import torch
    ...
    cfg = yaml.safe_load(Path("/config/rhyme_training.yaml").read_text())
    ...
    save_path = Path(ckpt_cfg.get("save_path", "/vol/checkpoints/poet_rhyme/"))

    train_file = Path("/vol/data/rhyme_train.jsonl")
    valid_file = Path("/vol/data/rhyme_valid.jsonl")
    data_vol.reload()
    if not train_file.exists():
        raise FileNotFoundError(
            "Upload rhyme data first: python scripts/data_generation/prepare_rhyme_training_data.py && python scripts/modal/upload_data.py"
        )
    ...
    if not train_data:
        raise ValueError("No training data in rhyme_train.jsonl")
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    with app.run():
        path = train_rhyme_poet.remote(num_epochs_override=a.num_epochs_override)
        print(f"Done: {path}")
